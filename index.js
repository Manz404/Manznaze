...
async function startNaze() {
    ...
    naze.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Welcome To ${metadata.subject} @${num.split("@")[0]} Selamat Datang di , Jangan lupa intro ya dek👋` })
                } else if (anu.action == 'remove') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}, Nitip Kopi ya☕` })
                } else if (anu.action == 'promote') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Promote From ${metadata.subject}, Selamat😄` })
                } else if (anu.action == 'demote') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Demote From ${metadata.subject}, Kasian Di Demote🤭` })
              }
            }
        } catch (err) {
            console.log(err)
    ...
    naze.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`File Sesi Buruk, Harap Hapus Sesi dan Pindai Lagi`); naze.logout(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("Sambungan ditutup, sambungkan kembali ...."); startNaze(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Sambungan Hilang dari Server, menyambungkan kembali..."); startNaze(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Koneksi Diganti, Sesi Baru Lain Dibuka, Harap Tutup Sesi Saat Ini Terlebih Dahulu"); naze.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Perangkat Keluar, Silakan Pindai Lagi Dan Jalankan.`); naze.logout(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Diperlukan Mulai Ulang, Mulai Ulang..."); startNaze(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Waktu Koneksi Habis, Menghubungkan..."); startNaze(); }
            else naze.end(`Alasan Putus Tidak Diketahui: ${reason}|${connection}`)
        }
